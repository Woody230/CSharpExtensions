    ...
    where TSuccess: notnull
{
    ...
    public EitherResult<TNewFailure, TNewSuccess> Map<TNewFailure, TNewSuccess>(
    ...
    /// <param name="onSuccess">The delegate for transforming a success into a new success state.</param>
    /// <returns>The transformed result state.</returns>
    public EitherResult<TFailure, TNewSuccess> Map<TNewSuccess>(Func<TSuccess, TNewSuccess> onSuccess) where TNewSuccess: notnull
    {
    ...
    /// <param name="onFailure">The delegate for transforming a failure into a new failure state.</param>
    /// <returns>The transformed result state.</returns>
    public EitherResult<TNewFailure, TSuccess> Map<TNewFailure>(Func<TFailure, TNewFailure> onFailure) where TNewFailure: notnull
    {
    ...
    public EitherResult<TFailure, TSuccess> OnSuccess(Action<TSuccess> action)
    {
        if (IsSuccess)
        {
            action(Success);
    ...
    /// <returns>This result.</returns>
    public EitherResult<TFailure, TSuccess> OnFailure(Action<TFailure> action)
    {
        if (IsFailure)
        {
            action(Failure);
    ...
    /// <returns>This result.</returns>
    public EitherResult<TFailure, TSuccess> Apply(Action<TFailure> onFailure, Action<TSuccess> onSuccess)
    {
